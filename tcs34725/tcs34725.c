...
{
    uint16_t c_red,c_green,c_blue, red_nor, green_nor, blue_nor;

    c_red=(int)((double)color_str->red/color_str->clear*255); // normalize
    c_green=(int)((double)color_str->green/color_str->clear*255);
    c_blue=(int)((double)color_str->blue/color_str->clear*255);
    
   // red_nor = (c_red-60)/75*225;
   // green_nor = (c_green-108)/32*225;
   // blue_nor = (c_blue-107)/117*225;

    NRF_LOG_INFO("Clear : %d Red   : %d Green : %d Blue  : %d",color_str->clear ,c_red+60 , c_green +10, c_blue);
